...
def test_generate_cypher():
    ...
    assert (
        cypher[0][:115]
        == 'MERGE (:ARTICLE {key:"9JHZ54TS", zotero_key:"9JHZ54TS" , title:"Frontotemporal Dementias" , url:"https://journals.l'
    )
    assert (
        cypher[-2]
        == 'MATCH (p1:PROCESS {name:"ulk1 expression"}), (p2:AGENT {name:"ulk1"})\nMERGE (p1)-[:GENERATES]->(p2);'
    )
    assert (
        cypher[-1]
...
def test_db_diff(sandbox_paths):
    ...
    assert (
        diff_cypher_lines[0][:98]
        == 'MERGE (:ARTICLE {key:"255SUP2B", zotero_key:"255SUP2B" , title:"Comparative interactomics analysis'
    )
    assert (
        diff_cypher_lines[136]
        == 'MATCH (p1:PROCESS {name:"atp6v1g1 expression"}), (p2:AGENT {name:"atp6v1g1"})'
    )
    assert (
        diff_cypher_lines[138]
        == 'MATCH (p1:PROCESS {name:"chaperone-mediated folding of atp6v1g1"}), (p2:AGENT {name:"atp6v1g1"})'
    )
...
def test_db_errors():
    ...
    ref = {
        ...
        ("5PJEZC9C", "A C9ORF72/SMCR8-containing complex regulates ULK1 "): [
            'Line: 21 Invalid relationship "~ ulk1" for "::process" ignored [::process autophagy @ ulk1 complex ~ ulk1]'
        ],
    }
...
def test_db_tools():
    ...
    ref = {
        "urls": ["https://www.uniprot.org/uniprot/O75385"],
        "tags": [],
        "notes": ["ulk1 is phosphorylated by mtor"],
        "labels": [":protein"],
        "synonyms": ["ulk1", "atg1"],
    }
...
def test_synonym_checking():
    ...
    assert syn_check["synonym appears in different agents"] == {
