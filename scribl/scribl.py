...
def main(argv=sys.argv):
    ...
    gp_db.add_argument(
        "-g",
        "--graphdb",
        help="path to scribl graph database",
        default=False,
        required=True,
    )
    gp_db.add_argument(
        "-n", "--namedb", help="Name of your database", default="", required=False
    )
    gp_db.add_argument(
        "-c",
        "--curator",
        help="Person/group who curated your database",
        default="",
        required=False,
    )
    gp_db.add_argument(
        "-d",
        "--description",
        help="Description of your database",
        default="",
        required=False,
    )
    ...
    gp_zotero_xor.add_argument(
        "-z", "--zoterofile", help="path to Zotero input file", default=None
    )
    gp_zotero_xor.add_argument(
        "--zotero-library",
        metavar="LIBRARY_ID:TYPE",
        help="Zotero library access in the format 'LIBRARY_ID:TYPE', where LIBRARY_ID is an integer and TYPE is (either 'group' or 'user')",
        default=None,
    )
    gp_zotero_api = parser.add_argument_group(
        "Zotero library options. Only valid to use if --zotero-library also supplied"
    )
    gp_zotero_api.add_argument(
        "--zotero-api-key",
        metavar="ZOTERO_API_KEY",
        help="If accessing a private Zotero library, use this API_KEY. ",
        default=None,
    )

    gp_outputs = parser.add_argument_group("Optional generated outputs")
    gp_outputs.add_argument(
        "--cyphertextfile",
        help="optional path to Cypher text output file",
        default=None,
        required=False,
    )
    gp_outputs.add_argument(
        "--graphmlfile",
        help="optional path to GraphML output file",
        default=None,
        required=False,
    )
    gp_outputs.add_argument(
        "--networkx-fig",
        help="optional path to NetworkX generated figure with visualization of GraphML, must have an extension supported by matplotlib",
        default=None,
        required=False,
    )

    gp_checks = parser.add_argument_group("Optional checks")
    gp_checks.add_argument(
        "--check-synonyms", help="run synonym check", default=False, required=False
    )
    gp_checks.add_argument(
        "--check-agentlabels",
        help="run agent label check",
        default=False,
        required=False,
    )

    parser.add_argument(
        "-v", "--verbose", help="output verbosity", action="store_true", default=False
    )
    parser.add_argument(
        "--overwrite",
    ...
    if args.zotero_library:
        try:
            ...
            zotero_library_id = int(
                zotero_library_id
            )  # ensure first item is an integer
            zotero_api_key = args.zotero_api_key
        except ValueError:
            parser.error(
                "--zotero-library must contain an integer and either `group` or `user` separated by a colon, e.g. '5251557:group'"
            )
    ...
    try:
        gdb = GraphDBInstance(
            scribl_graphdb_path, overwrite=overwrite, verbose=verbosity
        )
    ...
    if default_zotero_export_path:
        ...
        gdb.import_zotero_csv(
            default_zotero_export_path, overwrite=overwrite, verbose=verbosity
        )
    elif zotero_library_id and zotero_library_type:
        # get CSV generated from remote library:
        gdb.import_zotero_library(
            zotero_library_id,
            zotero_library_type,
            zotero_api_key,
            overwrite=overwrite,
